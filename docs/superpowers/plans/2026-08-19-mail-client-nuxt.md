...
- `vercel.json` cron schedule: `0 8 * * *` on `/api/cron/sync` (spec §5), documented as requiring Vercel Pro (spec §2).
...
describe("assertCronAuthorized", () => {
  it("throws if CRON_SECRET is not configured", () => {
    expect(() => assertCronAuthorized("Bearer x", undefined)).toThrow(
      "CRON_SECRET",
    );
  });
  ...
  it("does not throw when the header matches", () => {
    expect(() =>
      assertCronAuthorized("Bearer right-secret", "right-secret"),
    ).not.toThrow();
  });
});
```
...
describe("assertMailboxAccess", () => {
  it("resolves when an access row exists", async () => {
    ...
    await expect(
      assertMailboxAccess(admin, "user-1", "mb-1"),
...
export function assertCronAuthorized(
  ...
  if (authHeader !== `Bearer ${secret}`) {
    const err = new Error("Unauthorized cron request") as Error & {
      statusCode: number;
    };
    err.statusCode = 401;
    throw err;
...
export async function assertMailboxAccess(
  ...
  if (!data) {
    const err = new Error("Forbidden: no access to this mailbox") as Error & {
      statusCode: number;
    };
    err.statusCode = 403;
    throw err;
...
function fakeAdminForFolders(existing: { id: string } | null) {
  ...
  const single = vi
    .fn()
    .mockResolvedValue({ data: { id: "new-folder-id" }, error: null });
...
describe("ensureFolderId", () => {
  it("returns the existing folder id without inserting", async () => {
    ...
    await expect(ensureFolderId(admin, "mb-1", "INBOX")).resolves.toBe(
      "existing-folder-id",
    );
...
export async function ensureFolderId(
  admin: SupabaseClient,
  mailboxId: string,
  name: string,
): Promise<string> {
...
function fakeAdmin() {
  ...
  const folderMaybeSingle = vi
    .fn()
    .mockResolvedValue({ data: { id: "folder-1" }, error: null });
...
describe("syncAllMailboxes", () => {
  ...
  it("fetches only UIDs above last_uid_seen, upserts the message, uploads the attachment, and advances last_uid_seen", async () => {
    ...
    expect(admin.from("mailboxes").update).toHaveBeenCalledWith({
      last_uid_seen: 101,
    });
  });
});
...
export async function syncMailbox(
  ...
  const client = new ImapFlow(
    buildImapConfig(mailbox, decrypt(mailbox.imap_password_encrypted)),
  );
...
describe("sendMailForUser", () => {
  ...
  it("rejects when the user has no access to the mailbox", async () => {
    const admin = fakeAdmin({
      hasAccess: false,
      smtpPasswordEncrypted: encrypt("pw"),
    });
    await expect(
      sendMailForUser(admin, "user-1", {
        mailboxId: "mb-1",
        to: "x@y.com",
        subject: "Hi",
      }),
  ...
  it("sends mail through nodemailer using the decrypted SMTP password", async () => {
    const admin = fakeAdmin({
      hasAccess: true,
      smtpPasswordEncrypted: encrypt("smtp-secret"),
    });
...
export default defineEventHandler(async (event) => {
  ...
  if (!body?.mailboxId || !body?.to || !body?.subject) {
    throw createError({
      statusCode: 400,
      statusMessage: "mailboxId, to and subject are required",
    });
  }
...
function fakeAdmin(rows: unknown[]) {
  const accessMaybeSingle = vi
    .fn()
    .mockResolvedValue({ data: { mailbox_id: "mb-1" }, error: null });
  const accessEq2 = vi.fn(() => ({ maybeSingle: accessMaybeSingle }));
  const accessEq1 = vi.fn(() => ({ eq: accessEq2 }));
  const accessSelect = vi.fn(() => ({ eq: accessEq1 }));
...
describe("listMessages", () => {
  it("returns messages for a mailbox the user can access, newest first, paginated", async () => {
    ...
    const result = await listMessages(admin, "user-1", {
      mailboxId: "mb-1",
      limit: 20,
      offset: 0,
    });
    ...
    expect(admin.order).toHaveBeenCalledWith("received_at", {
      ascending: false,
    });
...
export default defineEventHandler(async (event) => {
  ...
  if (!mailboxId)
    throw createError({
      statusCode: 400,
      statusMessage: "mailboxId is required",
    });
...
describe("getAttachmentSignedUrl", () => {
  it("returns a signed URL when the user has access to the owning mailbox", async () => {
    ...
    expect(result).toEqual({
      url: "https://signed.example/file.txt",
      filename: "file.txt",
    });
  });
  ...
  it("rejects when the user has no access to the owning mailbox", async () => {
    ...
    await expect(
      getAttachmentSignedUrl(admin, "user-1", "att-1"),
    ).rejects.toMatchObject({ statusCode: 403 });
  });
});
```
...
export async function getAttachmentSignedUrl(
  ...
  const { data: attachment, error } = await admin
    .from("attachments")
    .select(
      "id, storage_path, filename, message_id, messages!inner(mailbox_id)",
    )
    .eq("id", attachmentId)
    .single();

  if (error || !attachment) {
    const err = new Error("Attachment not found") as Error & {
      statusCode: number;
    };
    err.statusCode = 404;
    throw err;
  ...
  if (signErr || !signed) {
    const err = new Error(signErr?.message || "Could not sign URL") as Error & {
      statusCode: number;
    };
    err.statusCode = 500;
    throw err;
...
const { channel, on, removeChannel } = vi.hoisted(() => {
  ...
  const on = vi.fn(
    (_event: string, _filter: unknown, cb: (payload: { new: any }) => void) => {
      capturedCallback = cb;
      return { subscribe };
    },
  );
...
describe("useMailRealtime", () => {
  it("prepends newly inserted messages to the list", async () => {
    const { useMailRealtime } =
      await import("../app/composables/useMailRealtime");
...
const { data: message } = await useAsyncData(
  `message-${messageId}`,
  async () => {
    const { data, error } = await supabase
      .from("messages")
      .select(
        "id, mailbox_id, from_addr, to_addrs, subject, body_html, body_text, received_at",
      )
      .eq("id", messageId)
      .single();
    if (error) throw error;
...
const { data: attachments } = await useAsyncData(
...
function startReply() {
  ...
  replySubject.value = message.value.subject
...
async function sendReply() {
  ...
  try {
    ...
    sendError.value =
      err?.data?.statusMessage || err?.message || "Could not send the message";
  } finally {
    sending.value = false;
  }
}
...
<template>
  <main class="message-view">
    ...
    <template v-if="message">
      ...
      <p class="meta">
        From: {{ message.from_addr }} · {{ message.received_at }}
      </p>
      ...
      <section v-if="attachments && attachments.length" class="attachments">
        ...
        <ul>
          <li v-for="att in attachments" :key="att.id">
            <a :href="attachmentUrl(att.id)" target="_blank" rel="noopener">{{
              att.filename
            }}</a>
      ...
      <section class="reply">
        <button v-if="!replying" type="button" @click="startReply">
          Reply
        </button>
        <form v-else @submit.prevent="sendReply">
          <label>To <input v-model="replyTo" type="email" required /></label>
          <label
            >Subject <input v-model="replySubject" type="text" required
          /></label>
          <label
            >Message <textarea v-model="replyBody" rows="6" required />
          </label>
...
4. `vercel.json`'s cron (`0 8 * * *`) requires the **Vercel Pro** plan —
